...
class ChatViewController: MessagesViewController,MessagesDataSource,MessagesLayoutDelegate,MessagesDisplayDelegate,InputBarAccessoryViewDelegate   {
    ...
    override func viewDidLoad() {
        ...
        self.messagesCollectionView.refreshControl?.addTarget(self, action: #selector(stopRefresh), for: .valueChanged)
    ...
    func StartChat(){
        ...
        self.db.collection("user").document(myUID!).collection("Messages").document(otherNickName).collection("Message").order(by: "time",descending: true).limit(to: 5).getDocuments{ (query, error) in
            ...
            for change in documentChange{
                ...
                if change.type == .added{
                    ...
                    if  change.document.data()["send"] != nil{
                        ...
                        self.messages.insert(Message(sender: self.currentUser, messageId: "1", sentDate: Date(), kind: .text("\( change.document.data()["send"] as! String)")),at: 0)
                    ...
                    if  change.document.data()["receive"] != nil{
                        ...
                        self.messages.insert(Message(sender: self.otherUser, messageId: "2", sentDate: Date(), kind: .text("\( change.document.data()["receive"] as! String)")), at: 0)
    ...
    @objc func StartChat2(){
        
        
        self.db.collection("user").document(myUID!).collection("Messages").document(otherNickName).collection("Message").order(by: "time", descending: true).limit(to: 5).start(afterDocument: self.lastDocument!).getDocuments{ (query, error) in
            if let querylast = query!.documents.last{
                self.lastDocument = querylast
            }
        ...
        //        let e = self.messagesCollectionView.indexPathForItem(at: CGPoint(x: 0, y: 40))
