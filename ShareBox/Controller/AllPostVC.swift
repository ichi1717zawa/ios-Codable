...
class allPostVC: UIViewController,UITableViewDelegate,UITableViewDataSource, UISearchBarDelegate,UISearchResultsUpdating,UITextFieldDelegate,UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        fetchData.shared.queryFirestore { (data) in
            self.data = data
            self.tableview.reloadData()
            self.lastTime = fetchData.shared.lastTime
            self.lastDocument = fetchData.shared.lastDocument
        }
//        fetchData.shared.testData()
//        queryFirestore()
    ...
    //MARK: -> refreshControlView
    func uiview () -> UIView {
        
        let uiview = UIView(frame: CGRect(x: 0, y: 0, width:tableview.frame.size.width, height: 60))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.data = fetchData.shared.data
        self.tableview.reloadData()
//        print(" 重新進入1",fetchData.shared.data)
//        print(" 重新進入1",self.tableview.reloadData())
    }
...
//                    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let ref = db.collection("userPost").document("\(postUUID)")
        
        ref.collection("views").getDocuments { (data, error) in
            guard let data = data else {return}
            let exist = data.documents.contains { (query) -> Bool in
                query.documentID == self.myUID
            }
            
            if exist == false {
                ref.collection("views").document(self.myUID).setData(["view":"view"]) { (error) in
                    guard error == nil else {return}
                    self.data[indexPath.row].viewsCount += 1
                    self.tableview.reloadData()
                    ref.updateData(["viewsCount":data.count + 1])
                }
            }
        ...
        db.collection("userPost").whereField("timeStamp", isGreaterThanOrEqualTo: date).getDocuments { (query, error) in
            ...
            for change in documentChange.documents{
                ...
                let postdetail = allPostModel(categoryImage: UIImage(named: "photo.fill")!,
                                              ...
                                              discription: change.data()["postIntroduction"] as? String ?? "N/A",
...
//            self.listenerToCheckNewPost()
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if contetHeight > scrollviewSizeHeight &&  offsetY   >= contetHeight - scrollviewSizeHeight {
            ...
            if !self.Pagination{
                self.tableview.tableFooterView = self.uiview()
                self.nextPageData()
            }
...
//        listener?.remove()
        db.collection("userPost").order(by: "timeStamp",descending: true).limit(to: 5).start(afterDocument: self.lastDocument!).getDocuments { (query, error) in
            ...
            //             guard let documentChange = query?.documentChanges else {return}
            guard let data = query?.documents  else {return }
            for i in data{
                let postdetail = allPostModel.QueryData2(Document:i )
                fetchData.shared.data.append(postdetail)
...
//                    self.getfavoriteListName()
    ...
    //MARK: -> listenerToCheckNewPost 監聽是否有新料
...
//                self.getfavoriteListName()
    ...
    var a = 255
    var time:Timer?
    @IBAction func 測試update(_ sender: Any) {
       
          
//        self.db.collection("userPost").document("\(documentID)").updateData(["viewsCount":[1,2,3,4,5,6]])
//        self.db.collection("userPost").document("\(documentID)").updateData(["viewsCount":Firebase.Firestore.Field])
        self.time = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(go), userInfo: nil, repeats: true)
    ...
    @objc func go (){
        let documentID = "F5F78820-82C0-4824-8BA0-AD4CED13B2E9"
        self.a += 1
        self.db.collection("userPost").document(documentID).updateData([  "viewsCount": FieldValue.arrayUnion([self.a]) ])
        print(self.a)
        
        
        
//        self.db.collection("userPost").document("\(documentID)").getDocument { (d, e) in
//            let e = d?.data()!["viewsCount"] as? NSArray
//            print(e!.count)
//    }
...
extension allPostVC : PostVCdelegate{
    func posted(postData: allPostModel) {
//        self.data.insert(postData, at: 0)
        
        fetchData.shared.data.insert(postData, at: 0)
//        self.tableview.reloadData()
        print("收到delegate")
        print(postData.posterUID)
        print(data.count)
